...
```text
阶段 1 采集输入 -> 阶段 2 生成 _dist -> AI Chat 生成并审核 Output -> 阶段 3 Memorize
```
...
在每周自动化线程中，完成 Weekly Output 并勾选候选后回复“继续生成记忆”或同义表达，即进入阶段 3。阶段 3 必须先确认 Output 非空且存在已勾选或用户明确确认的内容；没有确认内容时停止，不硬凑 Memory。
