...
export async function writeTimeWeekly(options = {}) {
  ...
  const timePath = path.join(outputRoot, "time.md");
  const calendarPath = path.join(outputRoot, "calendar.md");
  const suffix = `${process.pid}-${Date.now()}.tmp`;
  const timeTempPath = `${timePath}.${suffix}`;
  const calendarTempPath = `${calendarPath}.${suffix}`;
  await mkdir(outputRoot, { recursive: true });
  await Promise.all([
    writeFile(timeTempPath, renderTimeMarkdown(payload), "utf8"),
    writeFile(calendarTempPath, renderCalendarMarkdown(payload), "utf8")
  ]);
  await Promise.all([
    rename(timeTempPath, timePath),
    rename(calendarTempPath, calendarPath)
  ]);
  return { payload, timePath, calendarPath };
}
...
export function summarizeCalendar(range, events) {
  ...
  return { status: "available", daily, details, untagged, weeklyMinutes: daily.reduce((total, day) => total + day.minutes, 0), categories: Object.fromEntries(CATEGORIES.map((category) => [category, { minutes: daily.reduce((total, day) => total + day.categories[category].minutes, 0), blocks: daily.reduce((total, day) => total + day.categories[category].blocks, 0) }])) };
}
...
export function renderTimeMarkdown(payload) {
  const lines = [
    `# Time-X 屏幕时间摘要｜${payload.week}`, "",
    `- 时间范围：${payload.range.start} 至 ${payload.range.endExclusive}（不含结束时刻）`, `- 时区：${payload.timezone}`, `- 生成时间：${payload.generatedAt}`, "",
...
if (process.argv[1] === fileURLToPath(import.meta.url)) { const result = await writeTimeWeekly(parseArgs(process.argv.slice(2))); console.log(`Time weekly inputs written: ${path.relative(repoRoot, result.timePath)}, ${path.relative(repoRoot, result.calendarPath)}`); }
