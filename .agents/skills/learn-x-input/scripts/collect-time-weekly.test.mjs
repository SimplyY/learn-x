...
import { collectTimeWeekly, parseScreenTimeApps, renderCalendarMarkdown, renderTimeMarkdown } from "./collect-time-weekly.mjs";
...
test("summarizes Time-X tags and retains only applications over one hour", async () => {
  ...
  assert.deepEqual(payload.calendar.details[0], { date: "2026-06-08", start: "2026-06-08 09:00:00", end: "2026-06-08 10:00:00", title: "【健康】【关系】私人标题", description: "" });
  const calendar = renderCalendarMarkdown(payload);
  const time = renderTimeMarkdown(payload);
  assert.match(calendar, /【健康】1 小时/);
  assert.match(calendar, /详细时间/);
  assert.match(calendar, /私人标题/);
  assert.doesNotMatch(calendar, /Android 手机|全天|RSVP|忙闲|可见性|会议类型|accept|busy|no_meeting/);
  assert.match(time, /Android 手机/);
  assert.doesNotMatch(time, /私人标题|详细时间/);
});
...
test("parses Base application detail, keeps the strict threshold, and renames Mac ChatGPT", () => {
  ...
  const markdown = renderTimeMarkdown({ week: "2026-W24", range: { start: "2026-06-08", endExclusive: "2026-06-15" }, timezone: "Asia/Shanghai", generatedAt: "2026-06-15T00:00:00.000Z", screenTime: { status: "available", records: [{ device: "Android 手机", window: "近7日", capturedAt: "2026-06-14", totalMinutes: 300, source: "截图", apps: androidApps }, { device: "Mac", window: "近7日", capturedAt: "2026-06-14", totalMinutes: 300, source: "截图", apps: macApps }] } });
