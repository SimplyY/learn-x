...
- Set `LEARNX_FEISHU_HUMAN_EDITOR_ID` only to the history `editor_ids` value verified from the dedicated test document, and set `LEARNX_FEISHU_HUMAN_OPEN_ID` to the current user's `open_id` from the same canary. The history API and Search/Auth APIs currently return different ID spaces, so every real run checks the current `open_id` separately and never compares it directly to `editor_ids`; missing or inconsistent identity evidence fails closed.
...
- If one `revision_id` maps to more than one `history_version_id`, the collector records `needs_review` because the available fetch contract cannot prove which history snapshot belongs to which edit; it does not merge those events into a `ready` source.
