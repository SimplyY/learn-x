...
当用户要求「每周处理」「每周输出」「从 input 生成报告」「发现道法术候选」「做中学复盘」「行动闭环检查」「生成 weekly memory」「生成每周记忆」「压缩本周确认内容」时使用本 Skill。
...
- Weekly Output 默认不固定输出图谱、第一性原理、Prompt、Skill、写作或 Demo 候选；必要时才可在行动闭环中简短提及。
...
Weekly Output 默认围绕核心问题、做中学复盘、行动闭环、道 / 法 / 术、删除降权、下周 3 件事和来源覆盖。
...
- Prompt、Skill、图谱、写作、Demo 不是固定输出模块；只有强信号且服务行动闭环时才简短提及。
- 做中学复盘必须优先评估已经发生的行动和现实反馈。
