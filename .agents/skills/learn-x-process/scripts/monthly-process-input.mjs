...
const compressedSources = new Set(["ai", "research", "weread", "build", "build-bot", "coach", "podcast", "voice", "calendar"]);
const preservedTypes = new Set(["monthly-journal", "weekly", "daily", "flomo", "health", "time", "weekly-core", "weekly-munger"]);
...
export const monthlyCompressionPolicies = Object.freeze({
  ...
  calendar: Object.freeze({ signalToNoise: 0.15, minRatio: 0.08, maxRatio: 0.15 }),
...
export async function collectMonthlyProcessInput(monthId) {
  ...
  for (const file of files) {
    ...
    if (sourceName === "ai.generated") {
      source.status = "excluded";
      source.reason = "generated-temporary-not-chatpack";
      continue;
    }
...
export function isValidAiReview(content) {
  return validateAiReview(content).valid;
}

export function validateAiReview(content) {
  const text = String(content);
  const sectionCount = [...text.matchAll(/^##\s+.+$/gm)].length;
  const hasReviewContent = /(本周议题|本周反复思考的核心问题|核心洞察|精华议题摘要|精华问题摘要|最值得沉淀)/.test(text);
  ...
  const reasons = [];
  if (isPlaceholder(text)) reasons.push("placeholder");
  if (sectionCount < 2) reasons.push("too-few-sections");
  if (!hasPeopleAndEvents) reasons.push("missing-people-and-events");
  if (!hasReviewContent) reasons.push("missing-review-content");
  if (looksLikePrompt) reasons.push("prompt-like-output");
  return { valid: reasons.length === 0, reasons, sectionCount };
}
