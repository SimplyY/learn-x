...
- 用户说 `继续`、`周记和 AI 摘要已完成`、`继续采集周记` 或同义表达：只有 Stage 1.5 屏幕时间已写入 Base 后，才对同一目标周执行阶段 2；否则先索取三张截图。
...
目标：采集自动来源，提示飞书机器人侧自查，然后停止，等待用户完成人工周记和 AI 摘要。
...
- Android 总时长只按整小时估算；Mac 未手动切到上一完整自然周时拒绝写入。
- 截图不复制到 Learn-X、`03_input` 或 Base 附件；屏幕时间数据只存在于 Time-X Base。
- 成功后继续提示用户完成周记和 AI 摘要；用户回复 `继续` 才进入阶段 2。
