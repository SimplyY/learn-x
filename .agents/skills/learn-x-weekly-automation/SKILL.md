...
2. 采集本地自动来源，并提示飞书机器人侧自查：
   ...
   - 飞书日记与 AI Coach：执行 `npm run input:daily-coach -- --week YYYY-Www`。采集器先用 `--as bot` 现场核验 Base、表和字段，再服务端按目标周筛选并遍历全部分页；写入 `daily.md` 与 `coach.md`。只保留脱敏普通字段、URL、附件文件名/数量，不写联系方式、访谈原文或技术 record id。
