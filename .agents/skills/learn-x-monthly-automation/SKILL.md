...
5. 月度原始输入同时来自 `03_input/weekly/YYYY-Www/` 和 `03_input/monthly/YYYY-M/`；后者保存月记及其他月度独有来源。读取周目录时复用 `_source-status.json`：只有 `ready` 自动来源进入月度输入，`empty/failed/unavailable` 的旧文件保留但排除；缺失侧车兼容历史周，非法侧车失败关闭。另读取各周 `04_output/weekly/YYYY-WW.md` 中系统确认的标题 10「全文核心重点纪要」和标题 11「芒格之魂的洞察」，空项或 `todo` 跳过；不得读取其它周报正文代替原始输入。
   ...
   - 月度 `ai` 保留比例控制在 10%–30%；`build` 与 `build-bot` 各控制在 2%–8%。跨类型合并事件保留组合来源路径，并分别按原始来源字符审计。
   - 压缩不是凭感觉：每个需压缩来源带有 0–1 的信噪比，推荐保留比例按「范围下限 + 范围宽度 × 信噪比」计算；阶段 2 汇报必须同时展示信噪比、推荐比例、实际比例和硬范围。
...
8. 月度 Process Pack 以 Input 文件类型 / 来源标识为核心组织：保留完整性与缺口、按来源类型聚合的材料、来源与处理审计三层；同一类型跨多个周文件时合并到同一来源组，组内保留实际文件路径和事件边界。不得按“生命状态”“行动反馈”等语义主题重分组。来源内部标题必须降级，不能破坏目录树。对需要语义压缩的来源，同时生成 `_dist/monthly/YYYY-MM/compression-review/` 临时审阅目录，提供原始 Input 与压缩结果的可点击文件。
...
- 用户说 `继续`、`月记已完成`、`继续生成月报材料` 或同义表达：对同一目标月执行阶段 2。`继续` 不得绕过阶段 1 的“月内完整周记已确认”门槛；边界周只作提示，不阻塞。
...
1. 先执行 `npm run chatgpt:monthly-understanding -- --month YYYY-MM`。它通过 `chatgpt-web-bridge` 分两次独立的新聊天发送两个全量提示词；两次都不附本地基线或 Learn-X 内容，且两次请求默认间隔至少 5 分钟。两次都成功通过脱敏、字数校验后，才写入两个本地私有文件；当前文件之间的完整统一 diff 写入 `04_output/_dist/monthly/YYYY-MM/chatgpt-understanding.diff.md`，只用于本地审计，不作为 ChatGPT 输入或汇报正文。
...
4. 检查所有与目标月相交的周目录；实质性、已确认的 `weekly.md` 才进入“已确认周”范围。文件缺失、为空、模板化或仍含 `【待优化】AI 基础草稿` 时，不再阻塞阶段 1：月内完整周列为待人工补齐；首尾边界周只作醒目提示，不写入飞书月记，也不要求补齐。自动草稿不得把任何未确认周内容写成已确认事实。`_source-status.json` 非法、来源哈希或文件完整性无法判断时仍停止，避免把损坏数据当成普通缺口。
5. 检查完成后调用 `learn-x-monthly-journal`。它只从可安全使用的相交周与月度目录本地文件读取事实，按现有月度聚合的日期过滤、去重与边界规则生成“可编辑的部分草稿”；不得重新读取 Daily Base 或线上周记作为事实源。月内完整周缺口在汇报中醒目标记并保留给用户手动补写；边界周只在汇报中提示，不得写入飞书月记，不得猜测或伪造。
...
9. 停止并提示用户在飞书月记中编辑确认、补齐月内完整周的缺失内容、移除所有草稿标记，然后回复 `继续`。边界周只作醒目提示，不要求补齐、不写入文档；阶段 2 仍会再次检查月内完整周是否确认；压缩候选另行等待人工修改；定时任务不执行晋级。

阶段 1 汇报必须包含：目标月、ChatGPT 导出状态；成功时提供自我阅读版和 AI 记忆版的可点击本地文件链接，并基于实际变化压缩说明“新增、改动、删除”三类内容，不展开自我阅读版全文或完整 `diff` 代码。摘要必须来自本地前后版本，按语义归纳而不是逐行复述；无法判断的变化明确标为未知。以及月记草稿状态（含可点击飞书草稿链接）、已确认周范围、使用的本地来源、可点击的 `03_input/monthly/YYYY-M/` 路径、缺口、当前位置、下一步、再下一步。
...
2. 验证 `03_input/monthly/YYYY-M/monthly-journal.md` 是已确认定稿，不含 `【待优化】AI 基础草稿`；并再次验证所有月内完整周的 `weekly.md` 均已确认。首尾边界周缺失或未确认只记录为醒目提示，不阻止处理。旧 `weekly-inputs.md` 不参与处理。月记或月内完整周未确认时停止，不生成 `_dist`。
3. 首次运行：
...
阶段 2 汇报还必须把 `process-pack.md` 拆开说明，不能只报告总字数或事件总数。按实际生成的 Input 文件类型 / 来源组，逐组列出来源文件数、原始字符数、进入 Pack 的事件数和字符数；另外报告完整性与缺口、来源与处理审计两类固定部分的字符数和 UTF-8 字节数，并分别报告原始来源、确定性材料、语义压缩来源、最终上下文正文的来源数/事件数/字符数，明确数字口径。对每个语义压缩来源，提供 `compression-review/index.md` 和对应原文审阅文件的可点击链接。字符数以生成脚本使用的字符口径为准，字节数用于 100 KB 门禁核验。
