...
- 飞书文档、Base、云空间、日历、任务等操作必须走 `lark-cli`。
- 从第一性原理出发思考问题：回到问题最底层的基本元素和约束条件重新推导，而非依赖既有模板、惯例或类比。

## Harness 接入

- Learn-X 是个人上下文的唯一确认入口；只有人工确认后的 Memory 才能触发 YWNext 刷新。
- 其他仓库只消费通过校验的 `runtime/core-context/full.md`、`weighted.md` 或 `core.md`，不直接读取 `full-full.md`；个人上下文只影响个人化判断，不改变来源状态或写入门禁。
...
| 月度核心议题工作台 | `docs/MONTHLY_CORE_QUESTION_WORKBENCH.md`、`docs/TECH.md`、`.agents/skills/learn-x-monthly-question-workbench/` | 运行或修改工作台脚本、处理月度研究文档 |
