...
> 回答「系统现在真实是什么样」。由技术架构与规划节点在架构变化时增量维护。首次建立：2026-09-24（Case：[wechat-evidence-source](cases/wechat-evidence-source/case.md)）。最后核对：2026-09-24。
...
```text
03_input/weekly/YYYY-Www/          每周扁平输入目录：各来源单文件（daily/flomo/weread/…/wechat.md）+ _source-status.json 侧车
...
- 来源状态契约（`learn-x-input/scripts/lib/source-status.mjs`）：每来源一个状态（`ready/empty/needs_review/failed/unavailable`）＋规范文件名（`SOURCE_FILES`，wechat → `wechat.md`）。消费方按状态过滤输入文件。
- 周自动化（`learn-x-weekly-automation`）：阶段 1 采集自动来源并生成草稿 → 用户口令「周记已确认」进阶段 2 生成 `_dist` → 「继续」进阶段 3 验证后写 Memory。
- 备份（`learn-x-process/scripts/backup-weekly.mjs`）：`BACKUP_ROOTS = ["01_core", "03_input", "04_output"]` 整目录上传飞书云空间并存快照索引。
...
飞书（文档 / Base / 云空间备份 / 日历，统一走 `lark-cli`）、Flomo（Ego Lite 只读页面）、微信读书、Time-X 日历、Voice-X、AI Coach Base。**新增（PoC 评审中，未接入）**：TraceMemo 2.4.0 本机微信证据查询——经共享 `wechat-evidence` 只读适配器供各 Agent 查询；Learn-X 仅消费其派生的 `WeChat.md`。
