...
- 数量 8–12 条，覆盖不同书，同一本书最多 3 条；单书周改为同一章节最多 3 条，按章节分散。
- 每条候选 = **5–10 字概括 + 章节名 + 划线原文（引用块，一字不改）+ 1–2 个洞察问题（每个不超过 20 字，问向用户自己的处境，不做解释）**。不得改写、翻译或概括划线原文。

### 4. 创建飞书精读文档

- 每周新建一个独立文档：`lark-cli docs +create`，`--as user`；标题 `精读｜YYYY-Www`。
- 文档正文结构：**每本书一个一级标题 `# 《书名》｜作者`** 作为分节；候选为其下的二级标题 `## <5–10字概括>｜<章节名>`；同结构多书依次排布。
- 候选块结构（每候选同构）：

```markdown
# 《书名》｜作者

> 用法：值得深读的候选，直接在「我的思考」写下你自己的话（可加「可选行动」）；不填即视为跳过。只写你自己的理解，AI 不会替你写。
...
- **没有「跳过」字段**：候选的「我的思考」留空即视为未加工，读回时按此判定。
- 每次写入后回读校验（模式同 `learn-x-weekly-journal`：逐块写 → 回读 → 只使用本次回读确认的锚点生成直达链接）。
- **划线原文转义（实测必要）**：写入前对划线原文、候选标题与洞察问题内容做 Markdown 转义——`<` 无条件转义为 `\<`（否则会被当作 XML 标签解析吞掉）；内容行首的 `>`、`#`、`-`、`|`、有序列表前缀前加 `\`。创建后回读校验：每条「原始划线」必须能在回读 Markdown 中逐字找到；找不到时改用 XML 格式（`--doc-format xml`）重建该候选并再次回读，仍失败则报告并人工介入，不得静默继续。
...
- 校验：至少 1 条候选的「我的思考」非空；**全部为空 = 0 条**（合法，走 empty 分支）。
- 写 `03_input/weekly/<week>/jingdu.md`（原子写，结构与飞书文档同构）：
...
- **只转录用户写的我的思考/行动**；「我的思考」为空的候选不得写入正文（文末列「未加工候选」仅记 `概括｜章节名`）；AI 总结不得冒充用户认知。
