...
每次在有上下文版本中成功生成 Chat Pack，会记录当前子类型和已启用的非字数增强器使用次数。次数显示在提示词名称后，并在当前大类内按次数排序；次数低于 10 或位于后 50% 的子类型收进“其他提示词”下拉框。无上下文版本不记录使用次数。

使用次数不发送到第三方服务：本地桌面端写入本机待合并记录，公开有上下文版本写入浏览器 `localStorage`。公开版提供“复制使用记录”，用户将月度 JSON 交给 `learn-x-prompt-usage` Skill；Skill 校验通过后才合并、测试、提交和推送。统计基线只保存次数和合并月份，不保存问题正文或 Context。
