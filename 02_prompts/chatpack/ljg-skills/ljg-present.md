# ljg-present - AI Chat Prompt

> Source: .agents/skills/ljg-present/SKILL.md
> Purpose: 做分享稿、演讲稿、课件结构
...
---
name: ljg-present
description: "Use this skill to turn an outline into a single-file slogan-style HTML presentation saved under outputs/ljg-present/."
user_invocable: true
version: "3.0.0"
---
...
1. **获取内容**（文件 → 当前 agent 可用的文件读取能力 / 粘贴 → 直接用 / URL → 当前 agent 可用的网页读取能力）
2. **解析 outline**：
...
4. **应用映射规则**生成 slides 数组：
   ...
   - 强调 → 自动 hl
   - example 块 → 独立 pre 页
5. **读取** `assets/slogan_template.html`（cyber 主题需在模板基础上注入扫描线/HUD/光标 CSS）
6. **替换占位符**：
   - `{{TITLE}}` → 文件 `#+title:` 或显式参数
   - `{{SUBTITLE}}` → `#+author:` `#+date:` 拼接，或留空
   - `{{THEME}}` → 推断或显式参数（black|red|yellow|cyber）
   - `{{SLIDES_JSON}}` → JSON.stringify(slides)
7. **写文件**到 `outputs/ljg-present/{name}.html`（`{name}` 取自 `#+title:` 或文件名，去标点，≤ 20 字）
