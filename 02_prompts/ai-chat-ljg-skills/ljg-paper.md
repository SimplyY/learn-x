# ljg-paper - AI Chat Prompt

> Source: .agents/skills/ljg-paper/SKILL.md
> Purpose: 把研究论文讲成聪明的非专业读者能复述的故事
...
---
name: ljg-paper
description: "Use this skill to explain a research paper as a clear story for a smart non-specialist."
user_invocable: true
version: "5.0.0"
---
...
输出结构依据 `references/template.org`。禁止参考 `outputs/ljg-paper/` 中已有论文文件的章节结构——旧文件可能使用过期模板。
...
1. *中文母语凝练* — 像汪曾祺、王小波、阿城、李娟的标题：短、净、有刃。
   ...
   - 如当前项目内提供反翻译腔或写作模式参考文件，先读取对应文件做 Layer A 自查；没有则按本节约束自查。
...
- arxiv URL → 当前 agent 可用的网页读取能力
- PDF → 读取（注意 pages 参数限制）
- 本地文件 → 使用当前 agent 可用的文件读取能力
- 论文名称 → 当前 agent 可用的网页搜索能力

确保拿到：标题、作者、摘要、核心方法、结果。

如果论文有一张承载全文核心思路的总览图（overview / architecture diagram，通常是 Figure 1），提取并保存到 `outputs/ljg-paper/images/`，文件名 `{identifier}--paper-{简短标题}-overview.png`。

判断标准：这张图让人一看就抓住论文在做什么。不是所有论文都有——没有就跳过，不要硬找。

提取方法：
- arxiv → 访问 HTML 版（`arxiv.org/html/...`），找到图片 URL，当前 agent 可用的网页读取能力 下载
- PDF → 截取含图页面保存为图片
...
按 Denote 规范获取时间戳，读 `references/template.org`，写入 `outputs/ljg-paper/`。
